...
//  ChatApp
//
//  Created by Administrator on 5/17/19.
...
class ChatLogController: UICollectionViewController, UITextFieldDelegate, UICollectionViewDelegateFlowLayout,  UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let videoUrl = info[UIImagePickerController.InfoKey.mediaURL]{
         ...
         let uploadTask = ref.putFile(from: videoUrl as! URL , metadata: nil) { (metadata, error) in
                if error != nil{
                    ...
                    ref.downloadURL(completion: { (url, error) in
                        if error != nil{
                            ...
                            if  let thumbnailImage = self.thumbnailImageForFileUrl(fileUrl: videoUrl as! URL){
                                
                                let imageName = UUID().uuidString
                                let ref = Storage.storage().reference().child("profile_images").child("\(imageName).jpg")
                                
                                if  let uploadData = thumbnailImage.jpegData(compressionQuality: 0.1) {
                                    ...
                                    ref.putData(uploadData, metadata: nil, completion: { (_, err) in
                                        ...
                                        ref.downloadURL(completion: { (url, err) in
                                            ...
                                            self.sendMessageWithImageURL(imageUrl: imageUrl.absoluteString, image: thumbnailImage)
                                ...
                                //"imageUrl": imageUrl,
                                     let properties = ["imageWidth": thumbnailImage.size.width, "imageHeight": thumbnailImage.size.height, "storageUrl": storageUrl] as [String : AnyObject]
            ...
            if let selectedImage = selectedImageFromPicker {
                uploadToFirestoreStorageUsingImage(image: selectedImage)
            }
        }
    ...
    func uploadToFirestoreStorageUsingImage(image: UIImage){
        let imageName = UUID().uuidString
